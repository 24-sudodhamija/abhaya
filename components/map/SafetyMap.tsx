...
const SafetyMap = dynamic(() => import('./SafetyMapInner'), {
  ...
  loading: () => (
    <div
      className="relative w-full h-full min-h-[500px] rounded-3xl overflow-hidden border border-rose-950/40 bg-[#0a0104] flex flex-col items-center justify-center p-6 text-center shadow-2xl"
    ...
    >
      ...
      <span className="text-xs font-semibold text-rose-300 tracking-wider uppercase">
        Initializing Abhaya Dark Canvas Map...
      </span>
      <span className="text-[11px] text-zinc-500 mt-1 font-mono">
        OpenStreetMap Dark Canvas • Live Telemetry
      </span>
    </div>
  ),
});
